...
from src.utils.strategies import popen_global_rank_zero
...
log.warning(f"Supressed warnings: {PossibleUserWarning}")
...
_HYDRA_KWARGS = {
    ...
    "config_name": os.environ.get("HYDRA_CONFIG_NAME", "train.yaml")
}
@hydra.main(**_HYDRA_KWARGS)
def main(cfg: DictConfig) -> str:
    # Parse configuration in a user-defined manner
    ...
    datamodule.setup()
    os.environ["TRAIN_SAMPLES"] = str(datamodule.information()["length_train"])
    ...
    # Fitting
    trainer.fit(model, datamodule, ckpt_path=os.environ.get("LIGHTNING_CHECKPOINT_PATH", None))

    # Finalize
    loggers.finish()

    # Launch testing in subprocess
    if ("launch_testing" in cfg) and cfg.launch_testing:
        popen_global_rank_zero(["python", "scripts/test.py", "-u", str(os.environ["MLFLOW_RUN_UUID"])])
