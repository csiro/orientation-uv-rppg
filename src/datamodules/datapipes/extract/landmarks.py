...
from src.datamodules.datapipes.extract import ExtractorModel
from src.datamodules.datamodels.landmarks import LandmarksModel
from src.datamodules.datapipes.extract import InterpolateDetections

from src.datamodules.datasources.paths.detections import DetectionsSourceFile
from src.datamodules.datasources.files.detections import LandmarksKeys, LandmarksFormats
...
class LandmarkExtractor(ExtractorModel):
    """_summary_

    # TODO: Utilize LandmarksModel for formatting data into input format required (model consistent)
    """
    # Default attributes for Landmarks
    detection_type: str = LandmarksKeys.TYPE.value

    def __init__(self, *args, **kwargs) -> None:
        super(LandmarkExtractor, self).__init__(*args, **kwargs)


from src.datamodules.datamodels.landmarks import LandmarksModel

class InterpolateLandmarks(InterpolateDetections):
    """ Landmark data format is [[[frame_0, x, y, z], [frame_0, x, y, 1], x478], ...] = [N,478,4]
    """
    detection_model : LandmarksModel = LandmarksModel

    def __init__(self, *args, **kwargs) -> None:
        super(InterpolateLandmarks, self).__init__(*args, **kwargs)
...
# class ConvertLandmarksToMasks(DistributedDataOperation):
#     """ Convert landmarks to boolean facial segmentation masks using convex-hull
#     from landmark points.

#     # TODO: Rewrite ConvertLandmarksToMasks to suit new pipeline.

#     Args:
#         DistributedDataOperation (_type_): _description_
#     """
#     def __init__(self, subsets: Optional[List[int]] = None, outside: Optional[bool] = True, skip_existing: Optional[bool] = True, *args, **kwargs) -> None:
#         super(ConvertLandmarksToMasks, self).__init__(*args, **kwargs)
#         self.skip_existing = skip_existing
#         self.subsets = {val["contours"] for val in subsets.values()} if subsets is not None else None
#         self.outside = {val["outside"] for val in subsets.values()} if subsets is not None else None

#     def __call__(self, file: str) -> str:
#         with type(file)(file.path, "a") as fp:
#             model = fp.interface()
#             if "detections" in model.data.keys(): 
#                 for key, val in model.data["detections"].items():
#                     if val.attrs["type"] in ["landmarks"]:
#                         # conditionally convert
#                         name = f"detections/{key}_masks"
#                         if (name in fp.file) and self.skip_existing: 
#                             log.info(f"{self.__class__.__name__} Skipping {file.path.stem}")
#                             continue
#                         else:
#                             log.info(f"{self.__class__.__name__} Computing masks from landmarks for {path.stem}")
                        
#                         # read raw data from hdf5
#                         landmarks = val[:]
#                         frame_idxs = sorted(landmarks[:,0,0].astype(int))
#                         frames = np.stack([model.frames.data[frame_idx] for frame_idx in frame_idxs]) # issue when slicing with gaps (missing landmarks): must index in increasing order

#                         # convert raw into format
#                         frames = torch.tensor(frames).permute(0,3,1,2) #thwc to tchw
#                         landmarks = torch.tensor(landmarks)

#                         # convert landmarks to masks using convex hull of face
#                         if self.subsets is not None:
#                             start = True
#                             for subset, outside in zip(self.subsets, self.outside):
#                                 # first mask
#                                 if start:
#                                     masks = LandmarksModel.maskConvexHull(frames, landmarks, outside)
#                                     start = False

#                                 # subsequent masks
#                                 if outside:
#                                     masks &= LandmarksModel.maskConvexHull(frames, landmarks[:,subset,:], outside)
#                                 else:
#                                     masks |= LandmarksModel.maskConvexHull(frames, landmarks[:,subset,:], outside)
#                         else:
#                             masks = LandmarksModel.maskConvexHull(frames, landmarks, self.outside)

#                         # save to new dataset
#                         if name in fp.file:
#                             del fp.file[name]
#                         dataset = fp.file.create_dataset(name, data=masks.numpy())
#                         dataset.attrs.create("frame_indexes", frame_idxs)
#                         dataset.attrs.create("type", "boolean_mask")
        
#         return file                                          
    

def ConvertMediaPipeContoursToVertices(contour_set: str) -> List[int]:
    connections : FrozenSet = getattr(mediapipe.solutions.face_mesh, contour_set)
    vals = []
...
class MediaPipeLandmarkExtractor(LandmarkExtractor):
    ...
    Docs: https://github.com/google/mediapipe/blob/master/docs/solutions/face_mesh.md
    Cards: https://developers.google.com/mediapipe/solutions/vision/face_landmarker

    MediaPipe FaceMesh

    Establishes 3D metric-space and uses 468 3D facial landmark screen positions to estimate a face transform within that space.
    Face transform consists of face pose transformation matrix and tessellated facial mesh

    Pipeline consists of two real-time deep neural-networks
        BlazeFace: Detector which computes and tracks the facial position in the frame
        3DLandmarks: 3D Landmark model which predicts the approximate 3D surface via. regression

    Implemented as a MediaPipe graph which uses a 

    Face Transform Module: performs a single-camera land mark detection in the screen coordinate space/
        X/Y are normalized image coordinates
        Z is relative coordinate scaled as X under the weak perspective projection camera model

        Moves away from screen coordinate space towards a metric 3D space.
            Right-handed Orthnormal metric 3D coordinate space
            Virtual perspective camera located as space origin pointed in -ve Z axis
            Virtual camera params can be set freely but should be close to physical camera utilized
        Provides primitives to handle detected face as a metrical 3D object.

    Canonical Face Model
        Static 3D model of a human face which follows the 468 3D facial landmark topology used.
        
        Defines metric units as the scale of the canonical face defines the metric units of the metric 3D space (cm)
        Face pose transformation matrix is a linear map from the canonical face model into the runtime landmarks estimated per frame
            Hence assets can be modelled around the canonical space and properly aligned via the transformation matrix.

    Transform Pipeline
        Responsible for estimating the face transform objects within 3D metric space

        Face landmark screen coordinates are converted into the metric 3D space coordinates
        Face pose transformation matrix is estimated as a rigid linear mapping 
            From canonical face metric landmarks into runtime face metric landmarks (minimizes difference)
        Face mesh is created using runtime landmarks as vertex positions (XYZ)
            Both vertex texture coordinates and trigangular toplogy inheritend from canonical face model

    Args:
        LandmarkExtractor (_type_): _description_
    """
    # Attributes
    detection_format: str = LandmarksFormats.XYZ.value
    filename: str = f"FaceDetector_FaceMeshV2_{DetectionsSourceFile.LANDMARKS.value}" 
    algorithm : str = "FaceDetector_FaceMeshV2"

    def __init__(self, attrs: Optional[Dict[str,Any]] = {}, *args, **kwargs) -> None:
        mediapipe_fm = mediapipe.solutions.face_mesh
        model = mediapipe_fm.FaceMesh(*args, **kwargs)
        super(MediaPipeLandmarkExtractor, self).__init__(model, attrs)
    
    @property
    def attrs(self) -> Dict[str, Any]:
        class_attributes = super(MediaPipeLandmarkExtractor, self).attrs
        supplementary_attributes = {
            "contours": np.stack([list(val) for val in mediapipe.solutions.face_mesh.FACEMESH_CONTOURS]),
            "tesselation": np.stack([list(val) for val in mediapipe.solutions.face_mesh.FACEMESH_TESSELATION]),
        }
        return {**class_attributes, **supplementary_attributes}

    def preprocess(self, frame: ndarray) -> ndarray:
        """ Covnert frame from input into OpenCV format [HWC] (0-255) uint8

        Args:
            frame (ndarray): [C,H,W] in range [0-1] as type float32

        Returns:
            ndarray: [H, W, C] in range [0-255] as type uint8
        """
        # frame = (255 * frame).astype(np.uint8) # [0-1, fp32] -> [0-255, uint8]
        # frame = np.transpose(frame, axes=(1,2,0)) # [C,H,W] -> [H,W,C] (mediapipe)
        return frame
    ...
    def postprocess(self, results: Any, frame: ndarray, index: int) -> ndarray:
        ...
        Args:
            results (Any): MediaPipe.Results
            frame (ndarray): [C,H,W] in range [0-255] as type uint8
            index (int): Frame index

        Returns:
            List[TrackedObject]: _description_
        """
        # TODO: Append frame index to outputs for handling of multi-face tracking (maybe)
        outputs = []
        height, width = frame.shape[0], frame.shape[1]
        if results.multi_face_landmarks is None: return outputs
        for idx, detected_face in enumerate(results.multi_face_landmarks):
            # Extract (x,y,z) and scale to image size.
            x = [width * val.x for val in detected_face.landmark]
            y = [height * val.y for val in detected_face.landmark]
            z = [width * val.z for val in detected_face.landmark] # z-axis has same scale as x-axis
            f = [index for _ in range(len(x))]

            # Format into [f,x,y,z]
            output = np.transpose(np.stack([f, x, y, z]), axes=(1,0))
            output = output.astype(np.float32) # cast to fp32 (ensure not fp64)
...
# Obtained from mediapipe website
MEDIAPIPE_TESSELLATION = np.array([
...
MEDIAPIPE_UV_COORDINATES = np.array([
...
MEDIAPIPE_FACES = np.array([
    MEDIAPIPE_TESSELLATION[3*idx:3*(idx+1),0] for idx in range(MEDIAPIPE_TESSELLATION.shape[0]//3)
])
